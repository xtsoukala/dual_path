...
def create_cognate_or_false_friend_model_csv_files(results_dir, models=('cog1', 'cog2'), false_friends=False):
    for model_pair in [models, list(reversed(models))]:
        ...
        print(model_pair, len(model_pair))
        ...
        sentences_with = sentences_with.loc[(sentences_with.meaning == 1) &
                                            (sentences_with.point_of_interest_produced_last == False)]
        sentences_without = sentences_without.loc[(sentences_without.meaning == 1) &
                                                  (sentences_without.point_of_interest_produced_last == False)]
        baseline_sentences = baseline_sentences.loc[(baseline_sentences.meaning == 1) &
                                                    (baseline_sentences.point_of_interest_produced_last == False)]

        sentences_to_test = pd.concat([sentences_with, sentences_without, baseline_sentences])
        ...
        sentences_to_test.to_csv(f'{results_dir}/all_epochs_{"_".join(model_pair)}.csv')

        group_models(results_dir, f'all_epochs_{"_".join(model_pair)}.csv')


def group_models(results_dir, fname, make_exclusive=True):
    df = pd.read_csv(f'{results_dir}/{fname}', dtype={'switched_before': int, 'switched_at': int,
                                                      ...
                                                      'switched_after_anywhere': int, 'switched_before_es_en': int,
                                                      'switched_at_en_es': int, 'switched_right_after_es_en': int,
    ...
    df.to_csv(f'{results_dir}/exclusive_{fname}')
    gb = df.groupby(['epoch', 'network_num', 'model']).apply(lambda dft: pd.Series(
        {'code_switched': dft.is_code_switched.sum(),
         ...
         'switched_at_es_en': dft.switched_at_en_es.sum(),  # FIXME: rerun es_en
         ...
         'switched_before_es_en_percentage': dft.switched_before_es_en.sum() * 100 / len(dft.meaning),
         'switched_at_es_en_percentage': dft.switched_at_en_es.sum() * 100 / len(dft.meaning),
         'switched_right_after_es_en_percentage': dft.switched_right_after_es_en.sum() * 100 / len(dft.meaning),
         'switched_one_after_es_en_percentage': dft.switched_one_after_es_en.sum() * 100 / len(dft.meaning),
         'switched_after_anywhere_es_en_percentage': dft.switched_after_anywhere_es_en.sum() * 100 / len(dft.meaning),
         }))
    gb.to_csv(f'{results_dir}/exclusive_count_{fname}')


def create_all_model_csv_files(results_dir, create_csv=True):
    if create_csv:
        create_cognate_or_false_friend_model_csv_files(results_dir, models=('cog1', 'cog2'))
        create_cognate_or_false_friend_model_csv_files(results_dir, models=('ff1', 'ff2'), false_friends=True)

    for m in ['cog', 'ff']:
        df1 = pd.read_csv(f'{results_dir}/all_epochs_{m}1_{m}2_baseline.csv')
        df2 = pd.read_csv(f'{results_dir}/all_epochs_{m}2_{m}1_baseline.csv')
        pd.concat([df1, df2]).to_csv(f'{results_dir}/{m}_models_merged.csv', index=False,
                                     columns=['epoch', 'network_num', 'model', 'meaning', 'is_code_switched',
                                              ...
                                              'switched_at_en_es', 'switched_right_after_es_en',
...
if __name__ == "__main__":
    ...
    # create_all_model_csv_files(results_dir)

    group_models(results_dir, 'ff_models_merged.csv')
    plt = Plotter(results_dir=results_dir)
    # plt.performance_all_models(models=('cog1', 'cog2', 'ff1', 'ff2', 'baseline'))
    plt.plot_cognate_effect_over_time(df_name='exclusive_count_ff_models_merged.csv', ci=68)
