...
def create_model_csv_files(results_dir, models=('cog1', 'cog2'), model_name='cognate', only_last_epoch=False,
                           only_compare_with_sentences_not_containing_any_cognates=True, only_test_cognates=True):
    """
    :param results_dir:
    :param models:
    ...
    for model_pair in [models, list(reversed(models))]:
        ...
        if only_last_epoch:
            ...
            no_cog_or_ff_df = no_cog_or_ff_df[no_cog_or_ff_df.epoch == no_cog_or_ff_df.epoch.max()]
        ...
        sentences_with['sentence_idx'] = sentences_with.apply(lambda x:
                                                              hash_string(f'{x.message}{x.network_num}'),
                                                              axis=1)
...
def create_all_model_csv_files(results_dir, only_last_epoch=True, create_csv=True, models=('ff',),
                               per_switch_direction=False):
    if create_csv:
        for m in models:
            create_model_csv_files(results_dir, models=(f'{m}1', f'{m}2'), only_last_epoch=only_last_epoch)
