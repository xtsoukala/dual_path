...
def create_dataframes_for_plots(results_dir, epoch_from, epoch_to, simulation_range,
    ...
    for idx, group in enumerate(column_names):
        if auxiliary_experiment:
            df_performance = df.groupby(group).apply(lambda dft: pd.Series(
                {'meaning': dft.meaning.sum(),
                 ...
                 'total_perfect_sentences': len(dft[dft.message.str.contains("PERFECT")].meaning),
                 'total_progressive_sentences': len(dft[dft.message.str.contains("PROGRESSIVE")].meaning),
                 'l2_epoch': int(dft.l2_epoch.unique()[0]) if 'l2_epoch' in dft else 0,
                 'switched_auxiliary_perfect': dft[dft.message.str.contains("PERFECT")].switched_at_es_en.sum(),
                 'switched_participle_perfect': dft[
                     dft.message.str.contains("PERFECT")].switched_right_after_es_en.sum(),
                 'switched_auxiliary_prog': dft[dft.message.str.contains("PROGRESSIVE")].switched_at_es_en.sum(),
                 'switched_participle_prog': dft[dft.message.str.contains("PROGRESSIVE")].switched_right_after_es_en.sum(),
                 'meaning_perfect': dft[dft.message.str.contains("PERFECT")].meaning.sum(),
                 'meaning_progressive': dft[dft.message.str.contains("PROGRESSIVE")].meaning.sum(),
                 }))
        else:
            df_performance = df.groupby(group).apply(lambda dft: pd.Series(
                {'meaning': dft.meaning.sum(),
        ...
        if auxiliary_experiment:
            df_performance['meaning_perfect_percentage'] = (df_performance.meaning_perfect * 100 /
                                                            df_performance.total_perfect_sentences)
            df_performance['meaning_progressive_percentage'] = (df_performance.meaning_progressive * 100 /
            ...
            df_performance['switched_aux_perfect'] = (df_performance.switched_auxiliary_perfect * 100 /
                                                      df_performance.meaning_perfect)
            df_performance['switched_participle_perfect'] = (df_performance.switched_participle_perfect * 100 /
                                                             df_performance.meaning_perfect_percentage)
            df_performance['switched_aux_prog'] = (df_performance.switched_auxiliary_prog * 100 /
                                                   df_performance.meaning_progressive)
            df_performance['switched_participle_prog'] = (df_performance.switched_participle_prog * 100 /
                                                          df_performance.meaning_progressive_percentage)
            df_performance['switched_participle_prog_ALT'] = (df_performance.switched_participle_prog * 100 /
                                                          df_performance.meaning_progressive)
